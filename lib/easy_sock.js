...
Easysock.prototype.write = function (data, callback) {
    ...
    if (!this.config || !this.config.ip || !this.config.port) {
        callback("needs config info:ip,port");
    } else {
...
function initSocket(cur) {
    ...
    socket.on('connect', function () {
        ...
        if (!data || !Buffer.isBuffer(data) || data.length <= 0) {
            //error
            errorCall("receive error, illegal data");
            socket.end();
        } else {
 ...
 * 处理返回数据，回调
 */
function handleData(cur, buf, errorCall) {

    var obj = cur.decode(buf);

    if (typeof (obj) != "object") {
        //error
        errorCall("easy_sock:handle error:" + obj);
        cur.socket.destroy();
        return;
    }
