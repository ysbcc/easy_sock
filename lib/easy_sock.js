		...
		if (this.socket){				
			...
			if(this.config.timeout){
				timer = setTimeout(function(){
					...
					tryCloseSocket(self);
					callback("request timeout(" + self.config.timeout + "ms)");
				}, this.config.timeout);
			}
...
function initSocket(cur){
	...
	var connect_timer = setTimeout(function(){
		errorCall("easy_sock:TCP connect timeout(" + connect_timeout + ")");
	}, connect_timeout);
	...
	socket.on('connect',function(){
		...
		errorCall(new Error('socket error:' + e));
		socket.destroy();
		cur.socket = null;
    /*    
	}).on('end',function(){
		...
		//console.dir(socket);
        /* do nothing because it use setTimeout instead
        if (cur.currentSession > 0){
            errorCall(new Error('socket timeout'));
        }
        else{
...
function handleData(cur, buf){
	...
	if (typeof(obj) != "object"){
		//error
<<<<<<< HEAD
		console.log("easy_sock:handle error:" + obj);
=======
...
=======
		//error, 找不到上下文
		console.error("Can't find context. This should never happened!" + obj.seq);
		socket.destroy();
>>>>>>> origin/master
		return;
	}
...
=======
	...
	//if (!cur.config.keepAlive || (cur.calling_close && cur.currentSession == 0 && cur.tmpGetTaskList.length == 0)){ //fix an error that occur in short connection - 2015-12-03
